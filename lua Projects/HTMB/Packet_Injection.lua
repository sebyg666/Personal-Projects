...
function poke_warp_moogle(zone_number)
	local distance = 0
	if windower.ffxi.get_mob_by_index(moogles[zone_number]['NPC Index']) then
		distance = windower.ffxi.get_mob_by_index(moogles[zone_number]['NPC Index']).distance
		-- turn distance into yalms to match the distance addon
		distance = distance:sqrt()
		if distance > 0 and distance < 5 then
			local packet = packets.new('outgoing', 0x01A, {
				["Target"]=moogles[zone_number]['NPC'],
				["Target Index"]=moogles[zone_number]['NPC Index'],
				["Category"]=0,
				["Param"]=0,
				["_unknown1"]=0,
				['X Offset']=0,
				['Y Offset']=0,
				['Z Offset']=0,})
			notice('Attempting to buy Warp Scroll, sending poke!')
			packets.inject(packet)
		else
			activate_by_addon_moogle = false
			error('You are too far away from the NPC!')
		end
	else
		activate_by_addon_moogle = false
...
function create_0x05B_moogle(zone_number,option_index,message,HP_number,unknown_number)

	local info = moogles[zone_number]['0x05B']
	
	local packet = packets.new('outgoing', 0x05B)
		packet["Target"]=			info["Target"]
		packet["Option Index"]=		info["Option Index"]
		packet["_unknown1"]=		info["_unknown1"]
		packet["Target Index"]=		info["Target Index"]
		packet["Menu ID"]=			info["Menu ID"]
		packet["Zone"]=				zone_number
		packet["Automated Message"]=info["Automated Message"]
		packet["_unknown2"]=		info["_unknown2"]
	packets.inject(packet)
...
function create_0x083(zone_number)

	local info = moogles[zone_number]['0x083']
	local packet = packets.new('outgoing', 0x083)
		packet['Count'] = 		info['Count']
		packet['_unknown2'] = 	info['_unknown2']
		packet['Shop Slot'] = 	info['Shop Slot']
		packet['_unknown3'] = 	info['_unknown3']
		packet['_unknown4'] = 	info['_unknown4']
	packets.inject(packet)
