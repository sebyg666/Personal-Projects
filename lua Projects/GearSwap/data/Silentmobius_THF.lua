...
function user_setup()
	...
	state.DWMode 			= M{['description']='DW Mode','Normal','DW: 37+','DW: 6-11','DW: 5-0',}
	state.DWMode:set('DW: 5-0')
	...
	send_command('bind ^] gs c cycle DWMode')
...
function job_state_change(field, new_value, old_value)
	...
	if state.DWMode.value == 'Normal' then
		DW_needed = 0
		DW = false
	elseif state.DWMode.value == ('DW: 37+') then
		DW_needed = 37
		DW = true
	elseif state.DWMode.value == ('DW: 6-11') then
		DW_needed = 6
		DW = true
	elseif state.DWMode.value == ('DW: 5-0') then
		DW_needed = 0
		DW = true
	end
end
